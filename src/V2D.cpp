...
void V2D::getCoords(double& x, double& y) const
{
	x = this->x;
	y = this->y;
}
...
void V2D::setCoords(double x, double y)
{
	this->x = x;
	this->y = y;
}

void V2D::setX(double x){
	this->x = x;
}

void V2D::setY(double y){
...
V2D V2D::operator*(const double& d) const
{
	return V2D( (x*d),(y*d) );
}
...
double V2D::operator|(const V2D& v) const
{
	double x,y;
	v.getCoords(x,y);
...
void V2D::operator*=(const double d)
{
	x *= d;
	y *= d;
}
...
/*---------------------------------------------------------------------------*/
// external operators
V2D operator*(const double& d, const V2D& v)
{
	return (v * d);
}
